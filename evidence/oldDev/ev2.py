...
def show_instructions():
    additional_text = "Before we begin, please make sure you are in a quiet environment and can focus solely on the task.\nPress any key to continue."
    additional_stim = visual.TextStim(
        win, 
        text=additional_text, 
        height=20, 
        color='white')
    additional_stim.draw()
    win.flip()
    event.waitKeys()

    start_text = "In this experiment you can only press either X or M.\nPress Any Key to Start"
    ...
    more_text = "If the dot on the screen looks like it is on the left side of the screen press X. If it look like it is on the right side then press M\nPress any key to continue."
    more_stim = visual.TextStim(
        win, 
        text=more_text, 
        height=20, 
        color='white')
...
conditions = [-meanx] * half_trials + [meanx] * half_trials
