...
# Define variables
scale = 250
practice = 10 # number of practice trails
...
def makePracticeDotIndex(missingDot):
    tot = np.array(range(49))
    wot = np.delete(tot, missingDot)
    index = np.array(range(48))
    empty = np.sort(np.random.choice(index, 48, replace=False))
    return [wot[empty]]
...
#practice session
def practiceTrial():
    missingDot = random.choice(valid_points)
    [pDots] = makePracticeDotIndex(missingDot)
    dots=[]
...
visual.TextStim(win,"Welcome. Press any key to continue",height = 30).draw()
win.flip()
a=event.waitKeys()

visual.TextStim(win,"During the practice trial, you will see a fixation cross followed by a matrix. select the space with the missing dot. Press any key to continue",height = 30).draw()
win.flip()
a=event.waitKeys()

for i in range(practice):
	correct=practiceTrial()

visual.TextStim(win,"Just like in the practice trial, you will see a fixation cross followed by a matrix. Select the space with the missing dot. Press any key to begin trials",height = 30).draw()
win.flip()
a=event.waitKeys()
...
visual.TextStim(win,"Thank You",height = 30).draw()
win.flip()
